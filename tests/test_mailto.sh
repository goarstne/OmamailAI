#!/bin/sh
# The desktop handler must summon OmamailAI with the mailto: URL intact.
...
printed=$(OMAMAILAI_MAILTO_PRINT=1 sh "$script" 'mailto:jane@example.com?subject=Hi')
expected='omarchy-shell shell summon omamailai {"mailto":"mailto:jane@example.com?subject=Hi"}'
[ "$printed" = "$expected" ] || fail "expected:
$expected
got:
$printed"

quoted=$(OMAMAILAI_MAILTO_PRINT=1 sh "$script" 'mailto:jane@example.com?subject=Say "hi"')
echo "$quoted" | grep -q '"mailto":"mailto:jane@example.com?subject=Say \\"hi\\""' \
  || fail "a quote in the URL must be JSON-escaped, got: $quoted"

blank=$(OMAMAILAI_MAILTO_PRINT=1 sh "$script")
[ "$blank" = 'omarchy-shell shell summon omamailai {}' ] \
